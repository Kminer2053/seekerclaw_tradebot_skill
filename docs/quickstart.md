...
- 승인 명령은 가능하면 **`/approve <draft_scenario_id>`** 형식을 사용한다(대상 식별용).
...
- 운영자 알림 언어는 `memory/tradebot-operator-preferences.json`의 `notification_language`로 제어하고(`fallback_language`로 보조 언어 지정 가능), 값이 없으면 한국어(`ko`)를 기본으로 사용한다.
